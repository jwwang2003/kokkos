...
set(SUPPORTED_MACA_ARCHS XCORE1000)
set(CORRESPONDING_MACA_FLAGS xcore1000)
foreach(ARCH IN LISTS SUPPORTED_MACA_ARCHS)
  list(FIND SUPPORTED_MACA_ARCHS ${ARCH} LIST_INDEX)
  list(GET CORRESPONDING_MACA_FLAGS ${LIST_INDEX} FLAG)
  kokkos_arch_option(${ARCH} GPU "MXMACA GPU ${FLAG}" "KOKKOS_SHOW_MACA_ARCHS")
endforeach()
...
kokkos_option(IMPL_MACAGPU_FLAGS "" STRING "Set compiler flags for Maca GPUs")
kokkos_option(IMPL_MACAGPU_LINK "" STRING "Set linker flags for Maca GPUs")
mark_as_advanced(Kokkos_IMPL_MACAGPU_FLAGS)
mark_as_advanced(Kokkos_IMPL_MACAGPU_LINK)

global_set(KOKKOS_MACA_OPTIONS)
if(KOKKOS_ENABLE_MACA)
  global_append(KOKKOS_MACA_OPTIONS -x maca)
  set(MACA_ARCH_FLAG "-offload-arch")
  if(KOKKOS_ENABLE_MACA_RELOCATABLE_DEVICE_CODE)
    global_append(KOKKOS_LINK_OPTIONS -maca-link)
    global_append(KOKKOS_MACA_OPTIONS -fgpu-rdc)
    global_append(KOKKOS_LINK_OPTIONS -fgpu-rdc)
  endif()
if(KOKKOS_IMPL_MACAGPU_FLAGS)
  global_append(KOKKOS_MACA_OPTIONS "${KOKKOS_IMPL_MACAGPU_FLAGS}")
endif()

set(MACAGPU_ARCH_ALREADY_SPECIFIED "")
function(CHECK_MACAGPU_ARCH ARCH FLAG)
  if(KOKKOS_ARCH_${ARCH})
    if(MACAGPU_ARCH_ALREADY_SPECIFIED)
      message(
        FATAL_ERROR
          "Multiple MACA GPU architectures given! Already have ${MACAGPU_ARCH_ALREADY_SPECIFIED}, but trying to add ${ARCH}. If you are re-running CMake, try clearing the cache and running again."
      )
    endif()
    set(MACAGPU_ARCH_ALREADY_SPECIFIED ${ARCH} PARENT_SCOPE)
    if(NOT KOKKOS_ENABLE_MACA)
      message(WARNING "Given MACA GPU architecture ${ARCH}, but Kokkos_ENABLE_MACA is OFF. Option will be ignored.")
      unset(KOKKOS_ARCH_${ARCH} PARENT_SCOPE)
    else()
      set(KOKKOS_ARCH_${ARCH} ON PARENT_SCOPE)
      set(KOKKOS_ARCH_MACA_GPU ${FLAG} PARENT_SCOPE)
      if(NOT KOKKOS_IMPL_MACAGPU_FLAGS)
        global_append(KOKKOS_MACA_OPTIONS "${MACA_ARCH_FLAG}=${FLAG}")
        if(KOKKOS_ENABLE_MACA_RELOCATABLE_DEVICE_CODE)
          global_append(KOKKOS_LINK_OPTIONS "${MACA_ARCH_FLAG}=${FLAG}")
        endif()
      endif()
      list(APPEND KOKKOS_ENABLED_ARCH_LIST ${ARCH})
      list(REMOVE_DUPLICATES KOKKOS_ENABLED_ARCH_LIST)
      set(KOKKOS_ENABLED_ARCH_LIST ${KOKKOS_ENABLED_ARCH_LIST} PARENT_SCOPE)
    endif()
  endif()
endfunction()
...
foreach(ARCH IN LISTS SUPPORTED_MACA_ARCHS)
  list(FIND SUPPORTED_MACA_ARCHS ${ARCH} LIST_INDEX)
  list(GET CORRESPONDING_MACA_FLAGS ${LIST_INDEX} FLAG)
  check_macagpu_arch(${ARCH} ${FLAG})
endforeach()

if(KOKKOS_ENABLE_MACA AND KOKKOS_IMPL_MACAGPU_FLAGS AND NOT KOKKOS_ARCH_MACA_GPU)
  foreach(ARCH IN LISTS SUPPORTED_MACA_ARCHS)
    list(FIND SUPPORTED_MACA_ARCHS ${ARCH} LIST_INDEX)
    list(GET CORRESPONDING_MACA_FLAGS ${LIST_INDEX} FLAG)
    string(REGEX MATCH "(^|;| )${MACA_ARCH_FLAG}=${FLAG}($|;| )" MACA_FLAG_MATCH "${KOKKOS_IMPL_MACAGPU_FLAGS}")
    if(MACA_FLAG_MATCH)
      kokkos_set_option(ARCH_${ARCH} ON)
      set(KOKKOS_ARCH_${ARCH} ON)
      set(KOKKOS_ARCH_MACA_GPU ${FLAG})
      list(APPEND KOKKOS_ENABLED_ARCH_LIST ${ARCH})
      list(REMOVE_DUPLICATES KOKKOS_ENABLED_ARCH_LIST)
      break()
    endif()
...
if(KOKKOS_ENABLE_MACA AND KOKKOS_ENABLE_MACA_RELOCATABLE_DEVICE_CODE)
  if(KOKKOS_IMPL_MACAGPU_FLAGS)
    global_append(KOKKOS_LINK_OPTIONS "${KOKKOS_IMPL_MACAGPU_FLAGS}")
  endif()
  if(KOKKOS_IMPL_MACAGPU_LINK)
    global_append(KOKKOS_LINK_OPTIONS "${KOKKOS_IMPL_MACAGPU_LINK}")
  endif()
endif()
endif()
