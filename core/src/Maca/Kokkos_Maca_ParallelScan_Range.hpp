...
*  FileName : Kokkos_Maca_ParallelScan_Range.hpp
*  Author   : jwwang2003
...
#ifndef KOKKOS_MACA_PARALLEL_SCAN_RANGE_HPP
#define KOKKOS_MACA_PARALLEL_SCAN_RANGE_HPP
  ...
  inline void impl_execute(int block_size) {
    ...
    if (nwork) {
      ...
      const typename Analysis::Reducer& final_reducer =
          m_functor_reducer.get_reducer();
      m_scratch_space =
          reinterpret_cast<word_size_type*>(Impl::maca_internal_scratch_space(
              m_policy.space(), final_reducer.value_size() * m_grid_x));
      m_scratch_flags = Impl::maca_internal_scratch_flags(m_policy.space(),
                                                         sizeof(size_type) * 1);
  ...
  inline void execute() {
    ...
    if (block_size == 0) {
      Kokkos::Impl::throw_runtime_exception(
          std::string("Kokkos::Impl::ParallelScan< Maca > could not find a "
 ...
 public:
  ...
  inline void execute() {
    ...
    if (nwork && !Base::m_result_ptr_device_accessible) {
      ...
      DeepCopy<HostSpace, MacaSpace, Maca>(
