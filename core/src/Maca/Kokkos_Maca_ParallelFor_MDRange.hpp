...
*  FileName : Kokkos_Maca_ParallelFor_MDRange.hpp
*  Author   : jwwang2003
...
#ifndef KOKKOS_MACA_PARALLEL_FOR_MDRANGE_HPP
#define KOKKOS_MACA_PARALLEL_FOR_MDRANGE_HPP
...
class ParallelFor<FunctorType, Kokkos::MDRangePolicy<Traits...>, Maca> {
 public:
 ...
 public:
  ...
  inline void execute() const {
    using ClosureType = ParallelFor<FunctorType, Policy, Maca>;
      ...
      : m_functor(arg_functor),
        ...
        m_max_grid_size({
            static_cast<index_type>(
                m_policy.space().maca_device_prop().maxGridSize[0]),
            static_cast<index_type>(
                m_policy.space().maca_device_prop().maxGridSize[1]),
            static_cast<index_type>(
                m_policy.space().maca_device_prop().maxGridSize[2]),
        }) {
  ...
  static int max_tile_size_product(const Policy&, const Functor&) {
    using closure_type =
        ParallelFor<FunctorType, Kokkos::MDRangePolicy<Traits...>, Maca>;
    unsigned block_size = maca_get_max_blocksize<closure_type, LaunchBounds>();
    if (block_size == 0)
      Kokkos::Impl::throw_runtime_exception(
          std::string("Kokkos::Impl::ParallelFor< Maca > could not find a valid "
                      "tile size."));
    return block_size;
  }
};
