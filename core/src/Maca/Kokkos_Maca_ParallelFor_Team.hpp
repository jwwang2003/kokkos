...
*  FileName : Kokkos_Maca_ParallelFor_Team.hpp
*  Author   : jwwang2003
...
#ifndef KOKKOS_MACA_PARALLEL_FOR_TEAM_HPP
#define KOKKOS_MACA_PARALLEL_FOR_TEAM_HPP
...
class ParallelFor<FunctorType, Kokkos::TeamPolicy<Properties...>, Maca> {
 public:
 ...
 public:
  ...
  ParallelFor(FunctorType const& arg_functor, Policy const& arg_policy)
    ...
    if (m_team_size < 0) {
      ...
      if (m_team_size <= 0)
        Kokkos::Impl::throw_runtime_exception(
            "Kokkos::Impl::ParallelFor<Maca, TeamPolicy> could not find a "
    ...
    if (maxShmemPerBlock < shmem_size_total) {
      ...
      error << "Kokkos::parallel_for<Maca>: Requested too much scratch memory "
    ...
    if (m_team_size > max_size) {
      std::stringstream error;
      error << "Kokkos::parallel_for<Maca>: Requested too large team size. "
               "Requested: "
