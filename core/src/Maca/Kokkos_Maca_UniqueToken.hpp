...
*  FileName : Kokkos_Maca_UniqueToken.hpp
*  Author   : jwwang2003
*  Email    : wjw_03@outlook.com
*  Date     : Fri 17 Apr 2026 11:46:14 AM CST
================================================================*/

// SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception
// SPDX-FileCopyrightText: Copyright Contributors to the Kokkos project

#ifndef KOKKOS_MACA_UNIQUE_TOKEN_HPP
#define KOKKOS_MACA_UNIQUE_TOKEN_HPP
...
template <>
class UniqueToken<Maca, UniqueTokenScope::Global> {
 protected:
  View<uint32_t*, MacaSpace> m_locks;

 public:
  using execution_space = Maca;
  using size_type       = int32_t;

  explicit UniqueToken(execution_space const& = Maca())
      : m_locks(Kokkos::Impl::maca_global_unique_token_locks()) {}

 protected:
  // These are constructors for the Instance version
  UniqueToken(size_type max_size) {
    m_locks = Kokkos::View<uint32_t*, MacaSpace>("Kokkos::UniqueToken::m_locks",
                                                max_size);
  }
  UniqueToken(size_type max_size, execution_space const& exec) {
    m_locks = Kokkos::View<uint32_t*, MacaSpace>(
 ...
 private:
  __device__ size_type impl_acquire() const {
    ...
    while (active != done_active) {
      if (!done) {
        // Using m_locks(idx) fails self containment test of Kokkos_Maca.hpp
...
template <>
class UniqueToken<Maca, UniqueTokenScope::Instance>
    : public UniqueToken<Maca, UniqueTokenScope::Global> {
 public:
  // The instance version will forward to protected constructor which creates
  // a lock array per instance
  UniqueToken()
      : UniqueToken<Maca, UniqueTokenScope::Global>(Maca().concurrency()) {}
  explicit UniqueToken(execution_space const& arg)
      : UniqueToken<Maca, UniqueTokenScope::Global>(arg.concurrency(), arg) {}
  explicit UniqueToken(size_type max_size)
      : UniqueToken<Maca, UniqueTokenScope::Global>(max_size) {}
  UniqueToken(size_type max_size, execution_space const& arg)
      : UniqueToken<Maca, UniqueTokenScope::Global>(max_size, arg) {}
};

}  // namespace Experimental
}  // namespace Kokkos

#endif  // KOKKOS_MACA_UNIQUE_TOKEN_HPP
