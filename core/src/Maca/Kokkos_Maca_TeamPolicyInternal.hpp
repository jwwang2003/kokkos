...
*  FileName : Kokkos_Maca_TeamPolicyInternal.hpp
*  Author   : jwwang2003
...
#ifndef KOKKOS_MACA_TEAM_POLICY_INTERNAL_HPP
#define KOKKOS_MACA_TEAM_POLICY_INTERNAL_HPP
...
class TeamPolicyInternal<Maca, Properties...>
 ...
 public:
  ...
  inline int team_size_max(const FunctorType& f,
    ...
    using closure_type = Impl::ParallelReduce<
        ...
        TeamPolicy<Properties...>, Kokkos::Maca>;
  ...
  static int vector_length_max() { return MacaTraits::WarpSize; }
  ...
  inline static int scratch_size_max(int level) {
    // Maca Teams use (team_size + 2)*sizeof(double) shared memory for team
    ...
    size_t max_shmem = Maca().maca_device_prop().sharedMemPerBlock;
  ...
  TeamPolicyInternal()
      : m_space(),
        ...
        m_chunk_size(MacaTraits::WarpSize),
  ...
  TeamPolicyInternal(execution_space space, int league_size_,
    ...
    const int max_grid_size_x = m_space.maca_device_prop().maxGridSize[0];
    if (league_size_ >= max_grid_size_x)
      Impl::throw_runtime_exception(
          "Requested too large league_size for TeamPolicy on Maca execution "
          "space.");

    // Make sure total block size is permissible
    if (m_team_size * m_vector_length >
        static_cast<int>(MacaTraits::MaxThreadsPerBlock)) {
      std::stringstream error;
      error << "Kokkos::TeamPolicy<Maca>: Requested too large team size. "
               "Requested: "
            << m_team_size
            << ", Maximum: " << MacaTraits::MaxThreadsPerBlock / m_vector_length;
  ...
  using member_type = Kokkos::Impl::MacaTeamMember;
 ...
 protected:
  ...
  int internal_team_size_common(FunctorType const& f) const {
    ...
    if constexpr (BlockSize == BlockType::Max) {
      block_size = maca_get_max_team_blocksize<ClosureType,
                                              typename traits::launch_bounds>(
          space().impl_internal_space_instance(), functor);
    } else {
      block_size =
          maca_get_preferred_team_blocksize<ClosureType,
                                           typename traits::launch_bounds>(
              space().impl_internal_space_instance(), functor);
    }
    ...
    if (block_size == 0) {
      Kokkos::Impl::throw_runtime_exception(std::string(
          "Kokkos::Impl::ParallelFor/Reduce< Maca > could not find a valid "
          "team size."));
    }
...
__device__ inline int64_t maca_get_scratch_index(Maca::size_type league_size,
