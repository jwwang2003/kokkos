...
#ifndef KOKKOS_MACA_PARALLEL_REDUCE_TEAM_HPP
#define KOKKOS_MACA_PARALLEL_REDUCE_TEAM_HPP

#include <Kokkos_Parallel.hpp>

#include <Maca/Kokkos_Maca_KernelLaunch.hpp>
#include <Maca/Kokkos_Maca_Team.hpp>
#include <Maca/Kokkos_Maca_Instance.hpp>
#include <Maca/Kokkos_Maca_TeamPolicyInternal.hpp>
...
class ParallelReduce<CombinedFunctorReducerType,
 ...
 public:
  ...
  using word_size_type = std::conditional_t<
      sizeof(value_type) < sizeof(Kokkos::Maca::size_type),
      std::conditional_t<sizeof(value_type) == 2, int16_t, int8_t>,
      Kokkos::Maca::size_type>;
 ...
 public:
  ...
  __device__ inline void operator()() const {
    ...
    if (m_scratch_size[1] > 0) {
      threadid = maca_get_scratch_index(m_league_size, m_scratch_locks,
                                       m_num_scratch_locks);
    }
  ...
  __device__ inline void run(SHMEMReductionTag, int const threadid) const {
    ...
    if (do_final_reduce) {
      ...
      if (MacaTraits::WarpSize < word_count.value) {
        __syncthreads();
      }
  ...
  inline void execute() {
    ...
    if (!is_empty_range || need_device_set) {
      ...
      m_scratch_flags =
          maca_internal_scratch_flags(m_policy.space(), sizeof(size_type));
  ...
  ParallelReduce(CombinedFunctorReducerType const& arg_functor_reducer,
    ...
    if (m_team_size < 0) {
      ...
      if (m_team_size <= 0)
        Kokkos::Impl::throw_runtime_exception(
            "Kokkos::Impl::ParallelReduce<Maca, TeamPolicy> could not find a "
    ...
    if ((arg_policy.impl_vector_length() > 1) && !UseShflReduction)
      Impl::throw_runtime_exception(
          ...
          "greater than 1 is not currently supported for Maca for dynamic "
          "sized reduction types.");

    if ((m_team_size < MacaTraits::WarpSize) && !UseShflReduction)
      Impl::throw_runtime_exception(
          "Kokkos::parallel_reduce with a TeamPolicy using a team_size smaller "
          "than 64 is not currently supported with Maca for dynamic sized "
          "reduction types.");
    ...
    if (!Kokkos::has_single_bit(static_cast<unsigned>(m_team_size)) &&
        !UseShflReduction) {
      Kokkos::Impl::throw_runtime_exception(
          std::string("Kokkos::Impl::ParallelReduce< Maca > bad team size"));
    }
    ...
    if (maxShmemPerBlock < shmem_size_total) {
      ...
      error
          << "Kokkos::parallel_reduce<Maca>: Requested too much scratch memory "
    ...
    if (m_team_size >
        arg_policy.team_size_max_internal(m_functor_reducer.get_functor(),
                                          ...
                                          ParallelReduceTag())) {
      std::stringstream error;
      error << "Kokkos::parallel_reduce<Maca>: Requested too large team size. "
               "Requested: "
            << m_team_size << ", Maximum: "
