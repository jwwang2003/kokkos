...
*  FileName : Kokkos_Maca_ReduceScan.hpp
*  Author   : jwwang2003
...
#ifndef KOKKOS_MACA_REDUCESCAN_HPP
#define KOKKOS_MACA_REDUCESCAN_HPP
...
struct MacaReductionsFunctor<FunctorType, true> {
  ...
  __device__ static inline bool scalar_inter_block_reduction(
      FunctorType const& functor, Maca::size_type const block_count,
      Maca::size_type* const shared_data, Maca::size_type* const global_data,
      Maca::size_type* const global_flags) {
    ...
    constexpr unsigned int warp_size = Impl::MacaTraits::WarpSize;
...
struct MacaReductionsFunctor<FunctorType, false> {
  ...
  __device__ static inline bool scalar_inter_block_reduction(
    ...
    Scalar* shared_team_buffer_elements =
        reinterpret_cast<Scalar*>(shared_data);
    Scalar value        = shared_team_buffer_elements[threadIdx.y];
    int shared_elements = (blockDim.x * blockDim.y) / MacaTraits::WarpSize;
...
__device__ void maca_intra_block_reduce_scan(
  ...
  // warp reduction/scan.
  const bool is_full_warp = (((threadIdx.y >> MacaTraits::WarpIndexShift) + 1)
                             << MacaTraits::WarpIndexShift) <= blockDim.y;
    ...
    const unsigned mapped_idx =
        threadIdx.y + (is_full_warp ? 0
                                    : (not_less_power_of_two - blockDim.y) &
                                          (MacaTraits::WarpSize - 1));
    const pointer_type tdata_intra = base_data + value_count * threadIdx.y;
    const pointer_type warp_start =
        base_data + value_count * ((threadIdx.y >> MacaTraits::WarpIndexShift)
                                   << MacaTraits::WarpIndexShift);
    for (; (1 << bit_shift) < MacaTraits::WarpSize; ++bit_shift) {
  ...
  // Inter-warp reduce-scan by a single warp to avoid extra synchronizations
  {
    ...
    // (MacaTraits::WarpSize - 1) away from the warp start adress. For the
    ...
    const unsigned n_active_warps =
        ((blockDim.y - 1) >> MacaTraits::WarpIndexShift) + 1;
    if (threadIdx.y < n_active_warps) {
      const bool is_full_warp_inter =
          threadIdx.y < (blockDim.y >> MacaTraits::WarpIndexShift);
      pointer_type const tdata_inter =
          base_data +
          value_count * (is_full_warp_inter
                             ? (threadIdx.y << MacaTraits::WarpIndexShift) +
                                   (MacaTraits::WarpSize - 1)
                             : blockDim.y - 1);
      const unsigned index_shift =
          is_full_warp_inter
              ? 0
              : blockDim.y - (threadIdx.y << MacaTraits::WarpIndexShift);
      const int rtid_inter = (threadIdx.y << MacaTraits::WarpIndexShift) +
                             (MacaTraits::WarpSize - 1) - index_shift;
  ...
  if (DoScan) {
    ...
    const unsigned int WarpMask = MacaTraits::WarpSize - 1;
    const int is_last_thread_in_warp =
        is_full_warp ? ((threadIdx.y & WarpMask) == MacaTraits::WarpSize - 1)
                     : (threadIdx.y == blockDim.y - 1);
    if (threadIdx.y >= MacaTraits::WarpSize && !is_last_thread_in_warp) {
