...
*  FileName : Kokkos_Maca_ParallelFor_Range.hpp
*  Author   : jwwang2003
...
#ifndef KOKKOS_MACA_PARALLEL_FOR_RANGE_HPP
#define KOKKOS_MACA_PARALLEL_FOR_RANGE_HPP
...
class ParallelFor<FunctorType, Kokkos::RangePolicy<Traits...>, Kokkos::Maca> {
 public:
 ...
 public:
  ...
  inline void execute() const {
    ...
    using DriverType = ParallelFor<FunctorType, Policy, Kokkos::Maca>;
    ...
    if (block_size == 0) {
      Kokkos::Impl::throw_runtime_exception(
          std::string("Kokkos::Impl::ParallelFor< Maca > could not find a "
                      "valid execution configuration."));
    }
    const dim3 block(1, block_size, 1);
    const int maxGridSizeX = m_policy.space().maca_device_prop().maxGridSize[0];
    const dim3 grid(
        std::min(typename Policy::index_type((nwork + block.y - 1) / block.y),
                 typename Policy::index_type(maxGridSizeX)),
        1, 1);

    Kokkos::Impl::maca_parallel_launch<DriverType, LaunchBounds>(
